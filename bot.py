...
async def result(update: Update, context: CallbackContext) -> int:
    ...
    city_center_score = answers.count("1_a") + answers.count("2_a") + answers.count("3_a")
    business_area_score = answers.count("1_b") + answers.count("2_b") + answers.count("7_a")
    nature_score = answers.count("1_d") + answers.count("2_d") + answers.count("3_d")

    if city_center_score > business_area_score and city_center_score > nature_score:
        hotel = "Китай-город"
        url = "https://podkolokolny.ru"
    elif business_area_score > city_center_score and business_area_score > nature_score:
        hotel = "Сретенская/Курская"
        url = "https://sretenka36.ru" if "1_b" in answers else "https://kyrskaya.ru"
    elif nature_score > city_center_score and nature_score > business_area_score:
        hotel = "Глэмпинг"
        url = "https://norkerus.ru"
    else:
        hotel = "Бауманская/Менделеевская"
        url = "https://dobroslobodskaya6.ru" if "1_c" in answers else "https://obrazcova5.ru"

    # Отправляем результат
    keyboard = [
        [InlineKeyboardButton("Забронировать этот вариант 🏨", url=url)],
        [InlineKeyboardButton("Посмотреть другие варианты 🔍", url="https://bolotnikovskaya.ru")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
